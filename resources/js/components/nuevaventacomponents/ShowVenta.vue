                    ...
                    <b-container>
                        ...
                        <b-row class="mt-3">
                            <b-col cols="6">
                                <h5>
                                    <B>Sucursal:</B>
                                    {{ ventaData.inventario_name }}
                                </h5>
                                <h5>
                                    <B>Vendedor:</B> {{ ventaData.vendedor }}
                                </h5>
                    ...
                    <b-row>
                        <b-col cols="12">
                            <b-list-group>
                                ...
                                <b-list-group-item
                                    class="flex-column align-items-start"
                                    v-for="accesorio in ventaData.accesorios"
                                    :key="accesorio.id"
                                >
                                    <h5>{{ accesorio.otro.codigo }}</h5>

                                    <h5>{{ accesorio.otro.name }}</h5>

                                    <p>{{ accesorio.otro.description }}</p>

                                    <p align="right">
                                        <B>Precio:</B> ${{
                                            accesorio.pivot.price
                                        }}
                                    </p>
                                </b-list-group-item>
                                ...
                                <div>
                                    <b-list-group-item
                                        ...
                                        :key="transaction.id"
                                        :variant="
                                            transaction.taecel_success == false
                                                ? 'danger'
                                                : ''
                                        "
                                    >
                                        ...
                                        Compañia: {{ transaction.company.name
                                        }}<br />
                                        Numero: {{ transaction.dn }}<br />

                                        <div v-if="transaction.taecel == true">
                                            Mensaje:
                                            {{ transaction.taecel_message
                                            }}<br />

                                            <div
                                                v-if="transaction.taecel_folio"
                                            >
                                                Folio:
                                                {{ transaction.taecel_folio
                                                }}<br />
                                            </div>
                                        </div>
...
export default {
    ...
    methods: {
        open(ventaId) {
            this.loadData(ventaId);
            this.$root.$emit("bv::show::modal", "show-venta");
        },
        close() {
            this.$root.$emit("bv::hide::modal", "show-venta");
        },
        closeModal() {
            this.ventaData = {
                distribution: null,
